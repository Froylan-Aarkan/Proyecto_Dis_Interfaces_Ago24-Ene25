document.addEventListener("DOMContentLoaded", function () {
    const table = $('#estudiantes').DataTable();
    ...
    document.getElementById('consultar').addEventListener('click', function () {
        if (selectedRowData) {
            const matricula = selectedRowData[0]; // Primera columna: Matrícula
            ...
            window.location.href = `consultar_estudiante.html?matricula=${matricula}&nombre=${encodeURIComponent(nombre)}&correo=${encodeURIComponent(correo)}`;
        } else {
